...
Этот runbook хранит весь release-процесс плагина в репозитории `lidfly-plugins`. Текущее состояние версии `1.0.0` — черновик: плагин и marketplace ещё не отмечены как опубликованные, установщиков нет, `releases/latest.json` намеренно отсутствует.
...
- Windows-сборка подписана Authenticode;
