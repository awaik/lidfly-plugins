...
export function inspectPeMachine(bytes) {
  if (!Buffer.isBuffer(bytes) || bytes.length < 64) {
    throw new Error("Windows installer is too small to contain a PE header");
  }
  if (bytes.readUInt16LE(0) !== 0x5a4d) {
    throw new Error("Windows installer does not have an MZ header");
  }
  const peOffset = bytes.readInt32LE(0x3c);
  if (peOffset < 0 || peOffset > bytes.length - 6) {
    throw new Error("Windows installer has an invalid PE header offset");
  }
  if (bytes.readUInt32LE(peOffset) !== 0x00004550) {
    throw new Error("Windows installer does not have a PE signature");
  }
...
export function assertAmd64Pe(bytes) {
  ...
  if (inspected.machine !== PE_MACHINE_AMD64) {
    throw new Error(
      `Windows installer PE machine is 0x${inspected.machine.toString(16)}, expected AMD64 (0x8664)`,
    );
  }
  return inspected;
}
